...
def build_employee_equipment_map(inv_df):
    ...
    emp_equip = inv_df.groupby('employeeName')['equipmentType'].apply(lambda x: set(x.dropna().astype(str))).reset_index()
...
def load_data():
    ...
    text_cols = [
        'employeeName', 'equipmentType', 'brand', 'propertyNumber',
        'officeDivision', 'statusOfEmployment', 'natureOfWork',
        'accountablePerson', 'actualUser', 'office',
    ]
...
# =====================================================================
# PER-ITEM AI ANALYSIS FUNCTIONS
# =====================================================================

def classify_employee_priority_tiers(emp_map, repl_df=None):
    """
    Classify employees into priority tiers within each office:
    Priority 1 - No Desktop or Laptop (no ICT equipment at all)
    Priority 2 - Only peripheral devices (Printer, Scanner, Monitor, etc.)
    Priority 3 - Computer is older than 5 years
    Priority 4 - Computer is in Poor condition (health score < 40)
    Returns DataFrame with within-office ranking.
    """
    peripherals_set = {'Printer', 'Scanner', 'Monitor', 'UPS', 'CCTV / IP CAMERA', 'CCTV', 'Projector', 'Router', 'Switch'}
    results = []
    ...
    for _, row in emp_map.iterrows():
        ...
        has_laptop = any(v in equip_set for v in ['Laptop Computer', 'Laptop Computers'])
        has_computer = has_desktop or has_laptop
        current = ', '.join(sorted(equip_set)) if equip_set else 'None'
        ...
        if not has_computer:
            if not equip_set:
                results.append({
                    ...
                    'priority_tier': 1,
                    'priority_label': 'No Desktop or Laptop assigned',
                    'priority_score': 100,
                })
            elif equip_set.issubset(peripherals_set):
                results.append({
                    'employeeName': row['employeeName'],
                    'office': office,
                    'natureOfWork': row.get('natureOfWork', ''),
                    'current_equipment': current,
                    'priority_tier': 2,
                    'priority_label': f'Only peripheral devices: {current}',
                    'priority_score': 80,
                })
        elif repl_df is not None:
            emp_assets = repl_df[repl_df['actualUser'] == row['employeeName']] if 'actualUser' in repl_df.columns else pd.DataFrame()
            if not emp_assets.empty:
                oldest_age = emp_assets['equipment_age'].max() if 'equipment_age' in emp_assets.columns else 0
                worst_health = emp_assets['asset_health_score'].min() if 'asset_health_score' in emp_assets.columns else 100
                ...
                if has_old:
                    results.append({
                        ...
                        'current_equipment': current,
                        'priority_tier': 3,
                        'priority_label': f'Computer over {int(oldest_age)} years old',
                        'priority_score': 60,
                    })
                elif has_poor:
                    results.append({
                        ...
                        'current_equipment': current,
                        'priority_tier': 4,
                        'priority_label': f'Poor condition (health: {worst_health:.0f}/100)',
                        'priority_score': 50,
                    })
    ...
    if not result_df.empty:
        result_df = result_df.sort_values(
            ['office', 'priority_tier', 'priority_score'],
            ascending=[True, True, False]
        ).reset_index(drop=True)
        result_df['office_rank'] = result_df.groupby('office').cumcount() + 1
    return result_df


def get_item_employee_list(emp_recs, item_type, priority_tiers_df):
    """Get employee priority list for a specific item with within-office ranking."""
    if emp_recs is None or emp_recs.empty:
        return pd.DataFrame()
    item_recs = emp_recs[emp_recs['item'] == item_type].copy()
    if item_recs.empty:
        return pd.DataFrame()

    merged = item_recs.merge(
        priority_tiers_df[['employeeName', 'office', 'current_equipment', 'priority_tier', 'priority_label']],
        on=['employeeName', 'office'],
        how='left'
    )
    merged['priority_tier'] = merged['priority_tier'].fillna(1).astype(int)
    merged['current_equipment'] = merged['current_equipment'].fillna('None')
    merged['priority_label'] = merged['priority_label'].fillna('No Desktop or Laptop assigned')

    merged = merged.sort_values(['office', 'priority_tier']).reset_index(drop=True)
    merged['office_rank'] = merged.groupby('office').cumcount() + 1

    return merged[['office_rank', 'employeeName', 'office', 'current_equipment', 'priority_label', 'item', 'cost']]


def compute_item_office_priority(emp_map, item_type, selected_items):
    """Compute office priority ranking for Desktop or Laptop."""
    if item_type not in selected_items:
        return pd.DataFrame()

    need_label = 'Employees Needing Desktop' if item_type == 'Desktop Computer' else 'Employees Needing Laptop'
    cost = EQUIPMENT_COSTS.get(item_type, 50000)
    ...
    if not result.empty:
        result = result[result[need_label] > 0].sort_values(need_label, ascending=False).reset_index(drop=True)
        result['Rank'] = range(1, len(result) + 1)
        max_need = result[need_label].max()
        if max_need > 0:
            result['Priority'] = result[need_label].apply(
                lambda n: 'HIGH' if n >= max_need * 0.5 else ('MEDIUM' if n >= max_need * 0.2 else 'LOW')
            )
        else:
            result['Priority'] = 'LOW'
...
def compute_printer_office_priority(emp_map, office_counts):
    """Compute office priority ranking for Printer (shared resource)."""
    emp_per_division = emp_map['officeDivision'].value_counts().reset_index()
    emp_per_division.columns = ['office', 'unique_employees']
    largest_count = emp_per_division['unique_employees'].max()
    if largest_count == 0:
    ...
    rows = []
    for _, row_data in emp_per_division.iterrows():
        office = row_data['office']
        emp_count = row_data['unique_employees']
        target = round((emp_count / largest_count) * MAX_PRINTER_TARGET)
        target = max(1, target)
        existing = office_counts.get(office, {}).get('Printer', 0)
        recommend = max(0, target - existing)
        cost = recommend * EQUIPMENT_COSTS['Printer']

        if office == largest_office:
            reason = f"Largest office with highest workload ({emp_count} employees)."
        else:
            pct = (emp_count / largest_count) * 100
            reason = f"{pct:.0f}% of {largest_office}'s workforce ({emp_count} employees)."

        if existing > 0:
            reason += f" Has {existing} existing printer(s), needs {recommend} more."
        else:
            reason += f" Has no existing printers, needs all {recommend}."
        ...
        rows.append({
            ...
            'Additional Printers Needed': recommend,
            'Estimated Cost': cost,
            'Reason': reason,
        })
    ...
    result = pd.DataFrame(rows)
    if not result.empty:
        result = result.sort_values('Employees', ascending=False).reset_index(drop=True)
        result['Rank'] = range(1, len(result) + 1)
    return result


def generate_per_item_ai_explanation(item_type, emp_map, office_counts, desktop_count=0, laptop_count=0, printer_data=None):
    """Generate a standalone AI explanation for a single procurement item."""
    total_emps = len(emp_map)
    has_comp = emp_map['equipment_set'].apply(
        lambda s: any(v in s for v in ['Desktop Computer', 'Laptop Computer', 'Laptop Computers'])
    ).sum()
    wo_comp = total_emps - has_comp

    if item_type == 'Desktop Computer':
        emp_w_desk = emp_map['equipment_set'].apply(lambda s: 'Desktop Computer' in s).sum()
        emp_w_lap = emp_map['equipment_set'].apply(lambda s: any(v in s for v in ['Laptop Computer', 'Laptop Computers'])).sum()
        return (
            f"The AI detected **{wo_comp}** employees who do not have either a Desktop or Laptop assigned. "
            f"Out of **{total_emps}** total employees, **{emp_w_desk}** already have a Desktop "
            f"and **{emp_w_lap}** already have a Laptop. "
            f"These **{desktop_count}** employees were prioritized because they currently have only "
            f"peripheral devices or no ICT equipment. "
            f"**Mathematical Computation:** {desktop_count} employees × ₱{EQUIPMENT_COSTS['Desktop Computer']:,} = **₱{desktop_count * EQUIPMENT_COSTS['Desktop Computer']:,}**"
        )

    elif item_type == 'Laptop Computer':
        return (
            f"The AI recommends laptops only for employees selected under the laptop procurement policy "
            f"who currently have no assigned computer. "
            f"A total of **{laptop_count}** Laptop Computers are recommended for employees "
            f"with administrative, clerical, or other non-technical work. "
            f"**Mathematical Computation:** {laptop_count} employees × ₱{EQUIPMENT_COSTS['Laptop Computer']:,} = **₱{laptop_count * EQUIPMENT_COSTS['Laptop Computer']:,}**"
        )

    elif item_type == 'Printer':
        emp_per_division = emp_map['officeDivision'].value_counts()
        largest_office = emp_per_division.idxmax() if not emp_per_division.empty else ''
        largest_count = emp_per_division.max() if not emp_per_division.empty else 0
        return (
            f"The AI allocates printers based on office size rather than employee assignment. "
            f"**{largest_office}** has the largest workforce with **{largest_count:,}** employees "
            f"and therefore receives the maximum target allocation of **{MAX_PRINTER_TARGET}** shared printers. "
            f"Other offices receive proportionally scaled allocations: "
            f"Target = ROUND(Office Employees ÷ {largest_count} × {MAX_PRINTER_TARGET}). "
            f"Recommended = Target − Existing Printers (minimum 0)."
        )
